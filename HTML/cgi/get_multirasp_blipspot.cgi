...
if ( defined $plottime && $plottime =~ m|^([0-9][0-9][0-9][0-9](@[0-9][0-9][0-9][0-9])*)$|p ) { $plottime = ${^MATCH} ; } # alphanumeric+
...
if ( defined $parameter && $parameter =~ m|^([a-z][a-z0-9_ ]*(@[a-z][a-z0-9_ ]*)*)$|p ) {
    $parameter = ${^MATCH} ;  # alphanumeric+
}
else {
    $parameter = '';
}
...
my @spotlines = `/usr/bin/perl ${EXTRACTSCRIPT} $dataDir $region $ilat $klon 0 $plottime $parameter 2>&1`;
